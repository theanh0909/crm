<?php
    //Khai bao dang nhap
    $config['dn2_dnp'] = 'huyenthanh';
    $config['mk2_dnp'] = 'giamdoc9889@#';
        if ($_SERVER['PHP_AUTH_USER'] != $config['dn2_dnp'] || $_SERVER['PHP_AUTH_PW'] != $config['mk2_dnp']){
            header('WWW-Authenticate: Basic realm="Khai bao thong tin de dang nhap!"');
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>Công ty CP Giá Xây Dựng - Phần mềm, Đào tạo, Tư vấn</title>
...
//Khong biet ham $_GET['act] nay de lam

                if (isset($_GET['act']) == "do") {
                    $username = mysqli_real_escape_string($con, $_POST['username']) ? $_POST['username'] : "";
                    ...
                    if (!empty($_REQUEST['captcha'])) {
                        ...
                        if ($captcha_message == "Invalid captcha") {
                            $_SESSION['status'] = "";
                            header("Location: index.php?error=Sai mã kiểm tra rồi");
                            die();
                        }
                    } else {
                        $_SESSION['status'] = "";
                        header("Location: index.php?error=Bạn nhập SAI mã kiểm tra. Hãy nhập lại");
                        die();
                    }
                    $fb = check_username_password($username, md5($password));
                    if (!$fb) {
                        $_SESSION['status'] = "";
                        echo "<script language='JavaScript'> alert('Bạn hãy kiểm tra lại Username hoặc Password ');
                            location.replace('/')
                            </script> ";
                    } else {
            ...
            //</script>";
                        ...
                        if($chek_per == 1 || $chek_per == 2 ){
                            ...
                            echo
                            "<script language='JavaScript'> alert('Chào mừng {$username}, Bạn đã đăng nhập vào phần quản lý Key phần mềm ');
                            window.location.href ='views/send_key.php';
                            </script>";
                        }else echo
                        //Log_info('1',$username);
                            "<script language='JavaScript'> alert('Chào mừng {$username}, Bạn đã đăng nhập vào phần quản lý Key phần mềm ');
                ...
                <div id="body">
                    <div id="login-box" style="text-align:center">
                        ...
                        <form action="index.php?act=do" method="POST" style="margin-top: 80px">
                            <div id="login-box-name">Tài khoản:</div>
                                <div id="login-box-field"><table><tr><td><input name="username" class="form-login" title="username" value="<?php if (isset($_COOKIE['username'])){echo $_COOKIE['username'];} ?>" maxlength="2048" /></td></tr></table></div>
                        ...
                        <?php
                            ...
                            "<script language='JavaScript'> location.replace('views/administrator.php');
                            </script>";
                            }
                            } else {
                            echo '<div id="login-box" style="text-align:center"><span style="font-size:15px; color: blue; font-weight:600; margin-top:20px;">
                            Hệ thống đang tạm dừng để bảo dưỡng bạn hãy quay lại sau!
                            </div>';
                            die();
                            }
                        ?>
