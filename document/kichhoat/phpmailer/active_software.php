...
//$mail->AddAttachment($dirUpload . $attach['name']);
    if (!$kq) {
        ...
        // unlink($dirUpload . $attach['name']);
        $con = open_db();
        $sql = "UPDATE `giaxaydung_key`.`license` SET `stt_reg` = '1' ,`email_cus` = '$address' WHERE `id` = '$id'";
...
function send_email_2($address, $id, $title, $content) {
	$page_1 = $_SESSION['page'];
    $mail = new PHPMailer();
    /* =====================================
     * THIET LAP THONG TIN GUI MAIL
...
//$mail->AddAttachment($dirUpload . $attach['name']);
    if (!$kq) {
        ...
        echo "<meta http-equiv = \"refresh\" content=\"0;URL=key_no_reg.php\">";
		if($page_1 != ''){
                header('Location: http://giaxaydung.vn/kichhoat/Mod/Key.php?page='.$page_1);
               // echo "<meta http-equiv = \"refresh\" content=\"0;URL=key_no_reg.php?page=$page_1\>";
                } 
                else header('Location: http://giaxaydung.vn/kichhoat/Mod/Key.php');
    }
    return $kq;
}
...
function send_email_3($address, $title, $content,$n) {
    $con = open_db();
    $sql1 = 'SELECT * FROM `tbl_config_email` WHERE id= 1';
    $result1 = mysqli_query($con, $sql1);
    $row = mysqli_fetch_array($result1);
...
//Thiết lập nội dung chính của email
    ...
    //Thiết lập thông tin người nhận
    $i=0;
	$mail->addBCC($address[1]);
	$mail->addCC($address[0]);
    for($i=0;$i<$n;$i++){
		if($address[$i] != ''){
    ...
    if (!$kq) {
        ...
        //echo "<meta http-equiv = \"refresh\" content=\"0;URL=send_key.php\">";
		echo '<script type="text/javascript" charset="UTF-8">  
                      alert("Gửi email thành công");
                       window.location.replace("http://giaxaydung.vn/kichhoat/views/send_key.php");
                      </script>';
		//header('Location: http://giaxaydung.vn/kichhoat/views/send_key.php');
    }
    die();
    return $kq;
}
