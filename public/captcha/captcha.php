...
function open_db()
{
    ...
    $con = mysqli_connect($configs['DB_HOST'], $configs['DB_USERNAME'], $configs['DB_PASSWORD']);

    if (!$con) {
        die("Cannot connect to the server");
        return "";
    }
    $select_db = mysqli_select_db($con, $configs['DB_NAME']);
    if (!$select_db) {
...
$fp = fopen("tmp/captcha.txt", "w");

fwrite($fp, $_SESSION['captcha']."\n");
fclose($fp);
...
//Insert Or Update Captcha

$sqlcheck = mysqli_query($con, "select * from captcha where ip_address = '$client_ip' ");

if($sqlcheck && $sqlcheck->num_rows > 0) {
    $update = mysqli_query($con, "
        update captcha SET captcha_text = '$captcha_text' where ip_address = '$client_ip'
    ");
} else {
    mysqli_query($con, "insert into captcha(ip_address, captcha_text) values('$client_ip', '$captcha_text')");
}
...
class SimpleCaptcha {
    ...
    public function CreateImage() {
        ...
        echo        $this->WriteText($text, $fontcfg);
//        echo $this->captcha_text;
        $_SESSION[$this->session_var] = $text;


        //hiephv: luu text nay xuong file catcha.txt
        /** Transformations */
        $this->WaveImage();
    ...
    function GetDictionaryCaptchaText($extended = false) {
        ...
        if (substr($this->wordsFile, 0, 1) == '/') {
            ...
            $wordsfile = dirname(__FILE__) . '/tmp/'.$this->resourcesPath . '/' . $this->wordsFile;
        }
    ...
    protected function WriteText($text, $fontcfg = array()) {
        ...
        $fontfile = dirname(__FILE__) . '/tmp/resources/fonts/AntykwaBold.ttf';
