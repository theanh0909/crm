...
<div class="container">
    <div class="card card-custom">
        <div class="card-header py-3">
            ...
            <div class="card-toolbar">
                ...
                <div class="dropdown dropdown-inline mr-2">
                    <button type="button" class="btn btn-light-primary btn-primary font-weight-bolder" onclick="document.getElementById('export_email_today').submit()"><i class="la la-download"></i>Export</button>
         ...
         {!! Form::open(['route' => 'admin.license.export.sendEmailToday', 'method' => 'POST', 'id' => 'export_email_today', 'target' => '_blank']) !!}
                            {!! Form::close(); !!}
        ...
        <div class="card-header">
            <div class="row">
                {!! $licenses->appends(['query' => !empty($filters['query']) ? $filters['query'] : NULL, 'product_type' => $filters['product_type'], 'status' => $filters['status']])->links() !!}
            </div> 
            <table class="table">
                <thead>
                    <tr align="center">
                ...
                <tbody>
                    @foreach($licenses as $item)
                        ...
                        <tr id="row-{{$item->id}}">
                            <td>
                                @if($item->customer)
                                    <a href="{{route('admin.customer.edit', ['id' => $item->customer->id])}}">
            ...
            <div class="row">
                {!! $licenses->appends(['query' => !empty($filters['query']), 'product_type' => $filters['product_type'], 'status' => $filters['status']])->links() !!}
            </div>            
        </div>
    </div>
</div>
